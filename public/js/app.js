...
(function() {
  ...
  flavaApp.controller('SplashCtrl', function($scope) {
    ...
    $scope.onclick = function(e) {
      document.getElementById("siteEnter").currentTime = 0;
      document.getElementById("siteEnter").play();
    };
  ...
  flavaApp.controller('GameCtrl', function($scope, $routeParams) {
    ...
    _loadCallback = function() {
      ...
      //効果音
      document.getElementById("loadComplete").currentTime = 0;
      document.getElementById("loadComplete").play();
    ...
    _endCallback = function(score, log) {
      ...
      return $scope.$apply(function() {
        ...
        if ($scope.rank === "D") {
          $scope.result = "Failed";
          //効果音
          document.getElementById("resultFailed").currentTime = 0;
          document.getElementById("resultFailed").play();
        } else {
          if ($scope.score >= 100000) {
            $scope.result = "Perfect!!";
            //効果音
            document.getElementById("resultPerfect").currentTime = 0;
            document.getElementById("resultPerfect").play();
          } else {
            $scope.result = "Clear!";
            document.getElementById("resultClear").currentTime = 0;
            document.getElementById("resultClear").play();
          }
        }
        $scope.tweet = "http://twitter.com/intent/tweet?hashtags=幕BEAT&text=" + g_music[_gameId].title + " " + $scope.result + " score " + $scope.score + " rank " + $scope.rank + "&url=http://bakubeat.7fusigi.com/";

        if(storage.getItem(_gameId) != null){ 
          if (storage.getItem(_gameId) < score) {
  ...
  flavaApp.controller('SelectCtrl', function($scope) {
    ...
    $scope.mouseover = function(e) {
      //効果音
      document.getElementById("selectOver").currentTime = 0;
      document.getElementById("selectOver").play();
    };
    $scope.onclick = function(e) {
      //効果音
      document.getElementById("musicSelect").currentTime = 0;
