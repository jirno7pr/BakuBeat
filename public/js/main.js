...
(function() {
  ...
  this.Game = (function() {
    ...
    Game.prototype.start = function(music, loadCallback, startCallback, endCallback) {
      ...
      _game.preload(music.src,"music/btn_0.mp3","music/btn_1.mp3","music/btn_2.mp3","music/btn_3.mp3","music/btn_4.mp3","music/btn_5.mp3", music.img);
          console.log(music.src);
      _game.start();
      return _game.onload = function() {
        _endTime = music.endTime;
        _game.music = _game.assets[music.src];
        _game.btnse = [_game.assets["music/btn_0.mp3"],
                         _game.assets["music/btn_1.mp3"],
                         _game.assets["music/btn_2.mp3"],
                         _game.assets["music/btn_3.mp3"],
                         _game.assets["music/btn_4.mp3"],
                         _game.assets["music/btn_5.mp3"]];
    ...
    _endGameIfTimeOver = function() {
      var music;
      music = _game.music;
      if (music.currentTime >= _endTime || music.duration <= music.currentTime || _stop) {
//        music.volume = 1;
//        music.currentTime = 0;
        music.stop();
        _status = "stop";

//        while (_stop && _game.rootScene.childNodes.length > 0) {
//          _game.rootScene.removeChild(_game.rootScene.childNodes[0]);
//        }
        _stop = false;
  ...
  Note = (function() {
    ...
    Note.prototype.seek = function(keyCode) {
      ...
      if(code != null){
        var se = _game.btnse[code].clone();
        se.play();
        if(code == 77) code = null;
      }
      _ref = _group.childNodes;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        value = _ref[_i];
        if (value.key === code) {
          if ((-1 < (_ref1 = value.timing - _game.music.currentTime) && _ref1 < 1) && value.clear != true) {
            value.clear = true;
